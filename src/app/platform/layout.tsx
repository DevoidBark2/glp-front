...
const Layout: React.FC<LayoutProps> = ({ children }) => {
    ...
    return (
        <>
            {serviceMode ? <MaintenanceMode serviceModeText={footerInfo?.service_mode_text} /> : <>
                <HeaderBlock />
                {children}
                <footer className="bg-gray-800 text-white py-6 mt-6">
                    <div className="container mx-auto px-4">
                        <div className="flex flex-wrap justify-between items-center">
                            <div className="w-full sm:w-1/3 mb-4 sm:mb-0">
                                ...
                                <div className="flex items-center">
                                    {footerInfo?.logo_url && (
                                        <img src={`${nextConfig.env?.API_URL}${footerInfo?.logo_url}`} alt="Логотип" className="h-10 mr-4" />
                                    )}
                                    <div>
                            ...
                            <div className="w-full sm:w-1/3 mb-4 sm:mb-0">
                                ...
                                <p>Email: <Link type='mail' href={`mailto:${footerInfo?.support_email}`}>{footerInfo?.support_email}</Link></p>
                            </div>
                        ...
                        <div className="mt-6 text-center border-t border-gray-700 pt-4">
                            <p>&copy; {new Date().getFullYear()} Проект {footerInfo?.platform_name}. Все права защищены.</p>
                        </div>
                    </div>
                </footer>
